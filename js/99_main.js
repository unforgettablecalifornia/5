 ...
 */
 	//文字特效
	text_effect:function(now){
		// var now=car2._page.eq(car2._pageNext);//下一页
		$('.tx').removeClass('ef_show');//全局清理
		if(now.hasClass('text')){//判断是否有文字特效
			var tx=now.find('.tx');//当前页的div.tx是运动的容器
			setTimeout(text,500);//延迟500ms触发运动
		}
		var ms=0;
		function text(){
			tx.each(function(index,item){
				setTimeout(function(){
					$(item).hasClass('r')&&$(item).addClass('linearR ef_show');
 	...
 	// 页面切换停止
 	page_stop		: function(){
		car2._page.off('touchstart mousedown',car2.page_touch_start);
 		car2._page.off('touchmove mousemove',car2.page_touch_move);
 		car2._page.off('touchend mouseup',car2.page_touch_end);
 	},
 ...
 */
 	haddle_envent_fn : function(){
		...
		car2._on('success',function(){
			var next=car2._page.eq(car2._pageNext);//下一页，可能是上页，也可能是下页

			//第五页开屏广告
			if(car2._pageNow==3){
				if(typeof car2.p5=='undefined'){//只触发一次
					car2.fetchJs(4);//获取js脚本
			    }
				car2.p5=true;//是否触发的标示
			}

			//文本特效
			if(next.find('.m-mengban').length==0){//没有蒙版才触发文本特效
				//console.log(next)
				car2.text_effect(next);//文本特效
			}

			//蒙版特效
			car2.cover_draw(next);//蒙版特效结束后如果当前页有文字特效，就会触发文字特效

			//倒数第三页重载倒数第一二页
			if(car2._pageNow==car2._pageNum-6){
				console.log(car2['p'+(car2._pageNum-6)]);
				if(typeof car2['p'+(car2._pageNum-6)]!='once'){

					car2.fetchJs(5);
					car2.fetchJs(2);
				}
				car2['p'+(car2._pageNum-6)]='once';
			}
			//上滑时加载未加载的图片
			if(car2.if5){
 ...
 */
	...
	height_auto	: function(ele,val){
		...
		var height = car2.gdt?$(window).height():$(window).height();
...
//		car2.cover_draw(node,url,canvas_url,type,w,h,callback);
        car2.page_start();//翻页特效启动
        if(car2.if_mask){//如果是五分钟内跳转且进入页面是蒙版特效那么关掉翻页特效
        	car2.page_stop();
        }
        if(car2.if_page1_mask){
        	car2.page_stop();
        }
		// car2.start_callback();
 	...
 	// 蒙板插件初始化函数处理
 	cover_draw : function(now){
 		var mb=now.find('.m-mengban'),
 			index=now.index();
		if(mb.length===0||car2['canvas'+index]) return;

 		car2['canvas'+index]=true;
		var canvas_url=mb.attr('data-src'),
			type='image',
			w = 640,
			h = $(window).height(),
			callback=this.mengban_callback.bind(this,mb);
		var lottery = new Lottery(mb[0], canvas_url, type, w, h, callback);
		lottery.init();
		car2.page_stop();
	},

    mengban_callback: function(now){
    	var style=now.find('canvas')[0].style;
		style.cssText+=' ;-webkit-transition:all 0.6s ease;'
		style.opacity='0';
 		setTimeout(function(){
 			now.remove();
			car2.page_start();
			car2.text_effect(car2._page.eq(car2._pageNow));
 		},600);
	},
	// 蒙板插件回调函数处理
...
// 		},1500)
		...
		var node = $('#j-mengban')[0],
			...
			w = 640,
			h = $(window).height(),
			callback = car2.mengban_callback;
...
function backToIndex(){//看广告回退时跳转到计数页
    ...
    if(swith){//小于五分钟并且非首页才跳转到计数页
    	...
    	var currentPage=car2._page.eq(index);
    	if(!currentPage){
    		car2.lazy_start();

			if(car2._page.eq(0).find('.m-mengban').length==1){
				car2.if_page1_mask=true;
				car2.page_stop();
				car2.cover_draw(car2._page.eq(0));
			}
			return;
    	}
    	car2.if5=index;
        ...
        if(currentPage.find('.m-mengban').length==0){//如果没有蒙版了，可能得触发文本了
        	car2.text_effect(currentPage);
        }
        if(currentPage.find('.m-mengban').length>0){
        	car2.if_mask=true;
        	car2.cover_draw(currentPage);//触发蒙版特效
        }
        ...
        if(car2._page.eq(0).find('.m-mengban').length==0){//如果没有蒙版了，可能得触发文本了
        	car2.text_effect(car2._page.eq(0));
        }
    }
...
// 		$('.p-ct').after('<section style="position:absolute;bottom:0;height:100px;width:100%;background:black;overflow:hidden;font-size:0em;" id="gdt_banner"></section>');
// 	}else{
// 		$('.p-ct').after('<section style="position:absolute;bottom:0;height:100px;width:100%;background:black;overflow:hidden;font-size:0em;" id="gdt_banner"></section><img src="http://img0.hx.com/magazine/img/gdt_banner_close.png" id="gdt_banner_close" style="width:50px;height:35px;position:absolute;bottom:65px;right:0;display:block;opacity:0.6;z-index:1111;">');
// 		$('#gdt_banner_close').on('touchstart',function(){
// 			$(this).remove();
// 			$('#gdt_banner').remove();
// 			$('.p-ct').height($(window).height());
// 			$('.m-page').height($(window).height());
...
// 	function fetchAd(){
//         var script=document.createElement('script');
//         script.setAttribute('id','gdt-1030202025027661');
//         script.innerHTML="var TencentGDT = TencentGDT || [];"+
//                             "TencentGDT.push({"+
//                                 "posid:'1030202025027661',"+
//                                 "type:'banner',"+
//                                 "containerid:'gdt_banner',"+
//                                 "filltype:'full',"+
//                                 "appid:'1103442220'"+
//                             "});"+
//                             "(function() {"+
...
car2.hostname=window.location.hostname;
console.log(car2.hostname);